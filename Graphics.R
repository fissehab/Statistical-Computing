...
setwd("C:/Fish/data science/R_codes")
 ...
 data(iris)
  ...
  data(iris)
  r <- range(x)
  pairs(iris[,1:4],diag.panel = panel.d, col=iris$Species)
    
    
#     library(GGally)
#     data(iris)
     ggpairs(iris[101:150, 1:5], colour='Species', alpha=0.4)
   
    featurePlot(x=iris[,c("Sepal.Length","Sepal.Width","Petal.Length","Petal.Width")],
                y = iris$Species,plot="pairs")
    
    
    x <- scale(iris[101:150, 1:4])
    r <- range(x)
    pairs(x, diag.panel = panel.d, xlim = r, ylim = r, col=iris$Species)
...
### (Add elements to perspective plot)
...
### (Surface plot using wireframe(lattice))
